#!/usr/bin/env python3

from random import randint
import os
import sys

class Dot:

    def __init__(self, idx):
        self.idx = idx
        self.fr = randint(0,255)
        self.fg = randint(0,255)
        self.fb = randint(0,255)
        self.br = randint(0,255)
        self.bg = randint(0,255)
        self.bb = randint(0,255)
        self.glyph = "▤"
        # Neighbor indices pre-computed by Line
        self.ldot_idx = None
        self.rdot_idx = None
        self.opp_idx = None

    def get_str(self):
        return f"\x1b[38;2;{self.fr % 255};{self.fg % 255};{self.fb % 255}m\x1b[48;2;{self.br % 255};{self.bg % 255};{self.bb % 255}m{self.glyph}\x1b[0m"

    def evolve_with_neighbors(self, ldot, rdot, opp):

        # Store old values
        ofr, ofg, ofb = self.fr, self.fg, self.fb
        obr, obg, obb = self.br, self.bg, self.bb

        # Compute averages with the magical 3.976 divisor
        avgfr = (ofr + ldot.fr + rdot.fr + opp.fr) / 3.991
        avgfg = (ofg + ldot.fg + rdot.fg + opp.fg) / 3.991
        avgfb = (ofb + ldot.fb + rdot.fb + opp.fb) / 3.991
        avgbr = (obr + ldot.br + rdot.br + opp.br) / 3.991
        avgbg = (obg + ldot.bg + rdot.bg + opp.bg) / 3.991
        avgbb = (obb + ldot.bb + rdot.bb + opp.bb) / 3.991

        # Update with the magical 1.976 divisor
        self.fr = int((ofr + avgfr) / 1.991)
        self.fg = int((ofg + avgfg) / 1.991)
        self.fb = int((ofb + avgfb) / 1.991)
        self.br = int((obr + avgbr) / 1.991)
        self.bg = int((obg + avgbg) / 1.991)
        self.bb = int((obb + avgbb) / 1.991)

    def devolve_with_neighbors(self, ldot, rdot, opp):

        # Store old values
        ofr, ofg, ofb = self.fr, self.fg, self.fb
        obr, obg, obb = self.br, self.bg, self.bb

        # Compute averages with the magical 3.976 divisor
        avgfr = (ofr + ldot.fr + rdot.fr + opp.fr) / 4.009
        avgfg = (ofg + ldot.fg + rdot.fg + opp.fg) / 4.009
        avgfb = (ofb + ldot.fb + rdot.fb + opp.fb) / 4.009
        avgbr = (obr + ldot.br + rdot.br + opp.br) / 4.009
        avgbg = (obg + ldot.bg + rdot.bg + opp.bg) / 4.009
        avgbb = (obb + ldot.bb + rdot.bb + opp.bb) / 4.009

        # Update with the magical 1.976 divisor
        self.fr = int((ofr + avgfr) / 2.009)
        self.fg = int((ofg + avgfg) / 2.009)
        self.fb = int((ofb + avgfb) / 2.009)
        self.br = int((obr + avgbr) / 2.009)
        self.bg = int((obg + avgbg) / 2.009)
        self.bb = int((obb + avgbb) / 2.009)


class Line:

    def __init__(self):
        self.line = []
        width = os.get_terminal_size().columns

        # Create all dots
        for i in range(width):
            self.line.append(Dot(i))

        # Pre-compute neighbor indices for each dot
        for i, dot in enumerate(self.line):
            dot.ldot_idx = (i - 1) % width
            dot.rdot_idx = (i + 1) % width
            dot.opp_idx = abs(width - 1 - i)

    def evolve(self):
        """Evolve all dots in place"""
        for dot in self.line:
            dot.evolve_with_neighbors(
                self.line[dot.ldot_idx],
                self.line[dot.rdot_idx],
                self.line[dot.opp_idx]
            )

    def devolve(self):
        """Evolve all dots in place"""
        for dot in self.line:
            dot.devolve_with_neighbors(
                self.line[dot.ldot_idx],
                self.line[dot.rdot_idx],
                self.line[dot.opp_idx]
            )

    def display(self):
        # Compose string then write once
        sys.stdout.write(''.join(d.get_str() for d in self.line))
        sys.stdout.flush()

def do_art(in_len):
    line = Line()
    for i in range(in_len):
        line.evolve()
        line.display()
    for i in range(in_len):
        line.devolve()
        line.display()
