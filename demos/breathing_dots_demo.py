...
class BreathingDots(Simulation):
    """Simple demo: dots that pulse colors and move"""
    ...
    def step(self, frame: int):
        # Update colors (breathing effect)
        new_cells = [[None for _ in range(self.stage.width)] 
                     for _ in range(self.stage.height)]
        ...
        for y in range(self.stage.height):
            for x in range(self.stage.width):
                ...
                base_v = cell.metadata.get("v", 0.7)
                v = base_v + 0.2 * math.sin(phase + frame * 0.1)
                v = clamp01(v)
                ...
                # Move
                if frame % 10 == 0:  # Move every 10 frames
                    vx = cell.metadata.get("vx", 0)
                    vy = cell.metadata.get("vy", 0)
                    new_x = (x + vx) % self.stage.width
                    new_y = (y + vy) % self.stage.height
        ...
        # Apply movements
        if frame % 10 == 0:
            self.stage.cells = new_cells

# ===================== MAIN =====================

if __name__ == "__main__":
    with TerminalStage() as stage:
        sim = BreathingDots(stage)
