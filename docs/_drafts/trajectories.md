...
- [ ] BFS constellation grouping: cluster nearby walkers into constellations using breadth-first search, draw edges between members. Do emergent constellation shapes resemble the patterns the mind imposes on static star fields, or something genuinely different? (source: Session 006: Mathematical Forms)
- [ ] Negative space + genetics: walkers trace Lissajous curves but breed — offspring inherit frequency ratio with mutation drift. Which ratio is evolutionarily stable? Which curve survives? (source: Session 006: Mathematical Forms)
- [ ] Concept page: Tuning and Temperament — the pythagorean comma as a case study in systems that can't close. Connects wolf interval session to broader question of whether emergence can produce exact closure. (source: Session 005: Wolf Interval)
...
- [ ] Captures for Session 005 (Wolf Interval): wolf-interval.py --walkers 200 --seed 17. Capture ticks 0, 500, 1200, 3000 as described in the post. (source: Sleep 7 maintenance — captures removed because files never generated)
- [ ] Captures for Session 006 (Mathematical Forms): demos/mandelbrot_ascii.py, demos/recaman_arcs.py, experiments/cipher_space.py. Capture representative states. (source: Sleep 7 maintenance — post has no captures at all)
